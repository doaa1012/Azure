...
# Map table identifiers to actual models
PROPERTY_TABLE_MAP = {
...
def sample_typename_element_association_view(request):
    try:
        ...
        created_by_user = data.get("createdByUserId", None)  # Add this line
        properties_filter = data.get("properties", [])  # Expecting list of { name: ..., value: ... }
        ...
        if created_by_user:
            samples = samples.filter(field_createdby=created_by_user)
        ...
        for sample in samples:
            ...
            # ✅ Build a set of associated object IDs for property filtering
            associated_object_ids = set(
                link.linkedobjectid.objectid
                for link in associated_objects
                if link.linkedobjectid
            )

            # Include sample.objectid as well in the property check
            property_target_ids = list(associated_object_ids) + [sample.objectid]

            # Filter based on properties (if specified)
            all_matched = True
            for prop in properties_filter:
                table_key = prop.get("table")  # e.g., "propertyfloat"
                name = prop.get("name")
                value = prop.get("value")

                if not table_key or not name or value is None:
                    print(f"Skipping invalid property filter: {prop}")
                    continue

                model = PROPERTY_TABLE_MAP.get(table_key.lower())
                if not model:
                    print(f"Unknown property table: {table_key}")
                    continue

                matched = model.objects.filter(
                    objectid__in=property_target_ids,
                    propertyname=name,
                    value__icontains=value if isinstance(value, str) else value
                ).exists()

                if not matched:
                    print(f"Skipping Sample {sample.objectid} - Property '{name}' with value '{value}' not found in {table_key}")
                    all_matched = False
                    break
                ...
                if isinstance(elem, dict):  
                    ...
                    if comp:
                        if min_val is None and max_val is None:
                            ...
                            print(f"Skipping range check for {elem_name}, missing min/max values")
...
@csrf_exempt
def get_distinct_property_names(request, table_name):
    TABLE_MAP = {
        "propertystring": Propertystring,
        "propertyint": Propertyint,
        "propertyfloat": Propertyfloat,
    ...
    names = model.objects.values_list("propertyname", flat=True).distinct()
