                ...
                if elements:
                    for elem in elements:
                        if isinstance(elem, dict):
                            ...
                            comp = Composition.objects.filter(sampleid=sample_entry, elementname=elem_name).first()
                            if not comp or not (min_val <= comp.valuepercent <= max_val):
                                break
                        else:
                            if elem not in sample_elements:
                                break
                    else:
                        compositions = Composition.objects.filter(sampleid=sample_entry)
...
def download_all_samples(request):
    try:
        ...
        searched_typenames = data.get("typenames", [])  # Get typenames from search
        ...
        file_added = False  # Track if we add any files
        ...
        with zipfile.ZipFile(zip_root_path, "w", zipfile.ZIP_DEFLATED) as zipf:
            for sample_id in sample_ids:
                try:
                    ...
                    associated_objects = Objectlinkobject.objects.filter(
                        objectid=sample.objectid, 
                        linkedobjectid__typeid__typename__in=searched_typenames  # Filter only searched typenames
                    ).select_related("linkedobjectid")
                    ...
                    for obj in associated_objects:
                        ...
                        if linked_obj and linked_obj.objectfilepath:
                            ...
                            if os.path.exists(file_path):
                                ...
                                file_added = True  # Mark that at least one file was added
                            else:
                                print(f"⚠ File Not Found: {file_path}")  # Debugging missing files

                except Objectinfo.DoesNotExist:
                    print(f"⚠ Sample Not Found: {sample_id}")  # Debugging missing samples
                    continue  # Skip missing samples

        # If no files were added, return an error
        if not file_added:
            return JsonResponse({"error": "No matching files found for the selected samples and typenames."}, status=404)

        print(f"ZIP Created: {zip_root_path}")  # Debugging successful ZIP creation
